...
class UserTest(APITestCase):
    ...
    def test_get_all_post_created_by_user(self):
        self.client.force_login(self.basic_user)
        res = self.client.get("/user/posts/")
        data = res.data
        self.assertEqual(res.status_code, status.HTTP_200_OK)
...
#         follow_list = Follow.objects.filter(follower=self.basic_user).values_list(
#             "pk", flat=True
#         )
...
#         cls.follower = user_list[:3]
#         for user in cls.follower:
#             Follow.objects.create(follower=user, following=cls.base_user)
#         cls.following = user_list[2:]
#         for user in cls.following:
#             Follow.objects.create(follower=cls.base_user, following=user)
